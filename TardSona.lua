...
function Need:__init()
	...
	print("Hello ", myHero.name, ", TardSona v", self.Tard_version, " is ready to feed")
	...
	if _G.EOWLoaded then 
		self.Tard_Orb = 0; print("Eternal Orb Loaded") 
	elseif _G.SDK and _G.SDK.Orbwalker then 
		self.Tard_Orb = 1; print("IC Orb Loaded")		
...
require("Eternal Prediction")
  ...
  self.Tard_SonaSpells = { 
    [0] = {range = 850, delay = 0.25, speed = myHero:GetSpellData(0).speed, width = myHero:GetSpellData(0).width, damage = function(unit)
          local Tard_level = myHero:GetSpellData(0).level return Need:CalcMagicalDamage(myHero, unit, ({40, 70, 100, 130, 160})[Tard_level] + 0.5 * source.ap) end}, 
    [1] = {range = 1000, delay = 0,25, speed = myHero:GetSpellData(1).speed, width = myHero:GetSpellData(1).width, mana = 65},
    [2] = {range = 400, delay = 0.25, speed = myHero:GetSpellData(2).speed, width = myHero:GetSpellData(2).width},
    [3] = {range = 900, delay = 0.25, speed = 2400, width = myHero:GetSpellData(3).width, mana = 100, damage = function(unit)
          local Tard_level = myHero:GetSpellData(3).level return Need:CalcMagicalDamage(myHero, unit, ({150, 250, 350})[Tard_level] + 0.5 * source.ap) end}
  }
--[[
  self.Tard_SonaDmg = {
  ...
  if _G.Prediction_Loaded then self.Tard_EternalPred = true; print("Tosh Pred loaded ;)"); self.Tard_RPred = Prediction:SetSpell(self.Tard_QuinnSpells[3], TYPE_LINE, false) end
...
function TardSona:Tard_Combo()
  -- COMBO LOGIC HERE

    local Tard_target = Need:Tard_Target(850)    
    if Tard_target == nil then return end 
    if Need:Tard_IsValidTarget(Tard_target,850) then  
    ...
    if self.Tard_TardMenu.Draw.DrawReady:Value() then
      local Tard_Spell = Game.CanUseSpell     
        if Tard_Spell(_Q) == 0 and Tard_DrawMenu.DrawQ:Value() then
            Draw.Circle(myHero.pos, self.Tard_QuinnSpells[0].range, 1, Draw.Color(255, 96, 203, 67))
        end
        if Tard_Spell(_W) == 0 and Tard_DrawMenu.DrawW:Value() then
            Draw.Circle(myHero.pos, self.Tard_QuinnSpells[1].range, 1, Draw.Color(255, 255, 255, 255))
        end
        if Tard_Spell(_E) == 0 and Tard_DrawMenu.DrawE:Value() then
            Draw.Circle(myHero.pos, self.Tard_QuinnSpells[2].range, 1, Draw.Color(255, 255, 255, 255))
        end
