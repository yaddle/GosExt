	...
	function __TargetSelector:OnWndMsg(msg, wParam)
		if msg == WM_LBUTTONDOWN then
			if self.Menu.Advanced.SelectedTarget:Value() and not Utilities.MenuIsOpen then
				...
				for i = 1, #t do
					local hero = t[i];
					local mouse = _G.mousePos:Len() == 0 and cursorPos
					if mouse then
						local mouse2D  = Vector({x = mouse.x, y = mouse.y})
						local hero2D = hero.pos2D
						if Utilities:IsInRange(hero2D, mouse2D, 100) then
