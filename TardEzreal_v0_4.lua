...
function Need:Tard_GetEnemynearMouse()
    ...
    local Tard_Hero = Game.Hero(i)
        if not Tard_Hero.dead and not Tard_Hero.team == myHero.team and Need:Tard_GetDistanceSqr(Game.mousePos, Tard_Hero.pos) < 300 * 300 then
        return Tard_Hero
        end
    end
end
...
function TardEzreal:Tard_RonKey()
    if self.Tard_TardMenu.Misc.Rkey:Value() and Game.CanUseSpell(3) == 0 then
        local Tard_Rtarget = Need:Tard_GetEnemynearMouse()
