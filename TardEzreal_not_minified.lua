...
local TardTotalHero = TardHeroCount()
...
local Tard_EzrealSpells = {
    [0] = {range = 1200, delay = 0.25, speed = 2000, width = 80, spellType = TYPE_LINE, hitBox = true},
...
local Tard_HasBuff = function(unit, buffname)
    for i = 0, unit.buffCount do
        ...
        if Tard_Buff and Tard_Buff.name ~= '' and Tard_Buff.count > 0 and TardGameTimer() >= Tard_Buff.startTime and TardGameTimer() < Tard_Buff.expireTime and Tard_Buff.name == buffname then
...
local Tard_GetTarget = function(range)
    ...
    elseif Tard_Orb == 3 then
        if Tard_myHero.totalDamage > Tard_myHero.ap then
            local enemyHeroes_ADdmg = TardgsoObjects:GetEnemyHeroes(range, false, 'attack')
            Tard_target = TardgsoTS:GetTarget(enemyHeroes_ADdmg)
        else
            local enemyHeroes_APdmg = TardgsoObjects:GetEnemyHeroes(range, false, 'spell')
            Tard_target = TardgsoTS:GetTarget(enemyHeroes_APdmg, 1)
        end
    elseif Tard_Orb == 4 then
        if Tard_myHero.totalDamage > Tard_myHero.ap then
            local enemyHeroes_ADdmg = TardgsoObjects.enemyHeroes_attack
            Tard_target = TardgsoGetTarget(range, enemyHeroes_ADdmg, Tard_myHero.pos, false, false)
        else
            local enemyHeroes_APdmg = TardgsoObjects.enemyHeroes_spell
            Tard_target = TardgsoGetTarget(range, enemyHeroes_APdmg, myHero.pos, true, false)
        end
    else
...
local Tard_HP_PRED = function(unit, time)
    ...
    elseif Tard_Orb == 3 then
        return TardgsoFarm:MinionHpPredAccuracy(unit, time)
    elseif Tard_Orb == 4 then
...
local OnVision = function(unit)
    ...
    if _OnVision[unit.networkID].state == true and not unit.visible then
        ...
        _OnVision[unit.networkID].tick = TardTickCount()
    end
...
local GetPred = function(unit, speed, delay)
    ...
    if OnVision(unit).state == false then
        ...
        if Tard_GetDistanceSqr(unit.pos, predPos) > Tard_GetDistanceSqr(unit.pos, unit.posTo) then
            predPos = unit.posTo
        end
        return predPos
    else
...
local Tard_CastSpell =
    function(spell, pos, delay) -- Noddy CastSpell
...
local CalcMagicalDamage = function(source, target, amount)
    ...
    return TardMathMax(0, TardMathFloor(DamageReductionMod(source, target, PassivePercentMod(source, target, value) * amount, 2)))
end
...
local UpdateMovementHistory =
    ...
    for i = 1, TardTotalEnemy do
        ...
        if not _movementHistory[unit.charName] then
            ...
            _movementHistory[unit.charName]['StartPos'] = unit.pathing.pos
        ...
        if _movementHistory[unit.charName]['EndPos'].x ~= unit.pathing.endPos.x or _movementHistory[unit.charName]['EndPos'].y ~= unit.pathing.endPos.y or _movementHistory[unit.charName]['EndPos'].z ~= unit.pathing.endPos.z then
            _movementHistory[unit.charName]['PreviousAngle'] =
                Angle(
                TardVector(_movementHistory[unit.charName]['StartPos'].x, _movementHistory[unit.charName]['StartPos'].y, _movementHistory[unit.charName]['StartPos'].z),
                TardVector(_movementHistory[unit.charName]['EndPos'].x, _movementHistory[unit.charName]['EndPos'].y, _movementHistory[unit.charName]['EndPos'].z)
            )
            _movementHistory[unit.charName]['EndPos'] = unit.pathing.endPos
...
local GotSheen =
    function()
    if not Tard_TardMenu.B.Combo.sheen:Value() then
        return false
    end
    local Sheen =
        (GetItemSlot(Tard_myHero, 3057) >= 1 and GetItemSlot(Tard_myHero, 3057)) or (GetItemSlot(Tard_myHero, 3025) >= 1 and GetItemSlot(Tard_myHero, 3025)) or (GetItemSlot(Tard_myHero, 3100) >= 1 and GetItemSlot(Tard_myHero, 3100)) or
...
local Tard_CastQ = function(unit, collision, cancelifcollision)
    ...
    elseif TardPred == 2 then
        ...
        Qpos = GetPred(unit, 2000, 0.25 + TardLatency() * .001)
    end
    if Qpos and Qpos:DistanceTo() < 1200 then
        ...
        if cancelifcollision and Tard_Collision ~= 0 then
            return
        end
...
local Tard_CastW = function(unit)
    ...
    elseif TardPred == 2 then
        ...
        Wpos = GetPred(unit, 1600, 0.54 + TardLatency() * .001)
    end
...
local Tard_CastR = function(unit)
    ...
    elseif TardPred == 2 then
        ...
        Rpos = GetPred(unit, 2000, 1.76 + TardLatency() * .001)
...
local Tard_AutoQ = function(Mode)
    ...
    if TardQ then
        ...
        if target and TardMenu.CastOn[target.charName] and TardMenu.CastOn[target.charName]:Value() then
            ...
            if OutofRange and Dist < 1200 then
                Tard_CastQ(target)
            end
        end
    end
    ...
    for i = 1, TardTotalEnemy do
        ...
        if Tard_IsValidTarget(Tard_Hero) then
            ...
            if Tard_R_DMG and Tard_R_DMG > Tard_Hero.health + Tard_Hero.shieldAP then
                ...
                if Dist > Tard_AArange and Tard_HP_PRED(Tard_Hero, Dist / 2000) > 0 then
...
local Tard_Combo = function()
    ...
    if GotSheen() and Tard_target.distance <= 700 then
        return
    end
    if Tard_TardMenu.B.Combo.ComboQ:Value() and Tard_PercentMP(Tard_myHero) >= Tard_TardMenu.B.Combo.ComboQmana:Value() and TardIsRSpell(_Q) == 0 and Tard_IsValidTarget(Tard_target, 1200) then
        Tard_CastQ(Tard_target)
        Tard_CurrentTarget = Tard_target
    end
    if Tard_TardMenu.B.Combo.ComboW:Value() and Tard_PercentMP(Tard_myHero) >= Tard_TardMenu.B.Combo.ComboWmana:Value() and TardIsRSpell(_W) == 0 and Tard_IsValidTarget(Tard_target, 1050) then
...
local Tard_Harass = function()
    ...
    if Tard_TardMenu.B.Harass.HarassQ:Value() and Tard_PercentMP(Tard_myHero) >= Tard_TardMenu.B.Harass.HarassQMana:Value() and TardIsRSpell(_Q) == 0 and Tard_IsValidTarget(Tard_target, 1200) then
        if Tard_myHero.activeSpell.valid ~= true then
            Tard_CastQ(Tard_target)
            Tard_CurrentTarget = Tard_target
        end
    end
    if Tard_TardMenu.B.Harass.HarassW:Value() and Tard_PercentMP(Tard_myHero) >= Tard_TardMenu.B.Harass.HarassWMana:Value() and TardIsRSpell(_W) == 0 and Tard_IsValidTarget(Tard_target, 1050) then
        if Tard_myHero.activeSpell.valid ~= true then
            Tard_CastW(Tard_target)
            Tard_CurrentTarget = Tard_target
        end
...
local Tard_JungleClear = function()
    if not Tard_TardMenu.B.JungleClear.JungleQ:Value() or Tard_PercentMP(Tard_myHero) < Tard_TardMenu.B.JungleClear.JungleMana:Value() or Tard_myHero.attackData.state == 2 or TardIsRSpell(_Q) ~= 0 or IsEvading() then
        return
    end
    for i = 1, TardMinionCount() do
...
local Tard_LastHit = function()
    if not Tard_TardMenu.B.LastHit.LastHitQ:Value() or Tard_PercentMP(Tard_myHero) < Tard_TardMenu.B.LastHit.LastHitMana:Value() or Tard_AAstate == 2 or TardIsRSpell(_Q) ~= 0 or IsEvading() then
        return
    end
    local Tard_AArange = Tard_myHero.range + Tard_myHero.boundingRadius
    local Qlvl = Tard_myHero:GetSpellData(0).level
    ...
    function()
        ...
        if Tard_DrawMenu.dautoQ:Value() then
            ...
            if Tard_TardMenu.A.autoQ.ON:Value() then
                ...
                Tard_DrawText('Auto Q OFF', 20, Pos2D.x - 60, Pos2D.y + 30, Tard_DrawColor(255, 220, 050, 000))
            end
        end
    end
)
...
LocalCallbackAdd(
    ...
    function()
        ...
        elseif Tard_Orb == 5 then
            if Orbwalker.Enabled:Value() then
                print('Noddy rocks')
            else
                Tard_Orb = 6
                print("WARNING : you're not using any Orbwalker")
            end
        end
        for i = 0, 3 do
            if i == 0 then
                Tard_EzrealSpells[i].dmg = function(unit)
                    ...
                    return CalcPhysicalDamage(Tard_myHero, unit, ({15, 40, 65, 90, 115})[Tard_level] + 1.1 * Tard_myHero.totalDamage + 0.4 * Tard_myHero.ap)
                end
            elseif i == 1 then
                Tard_EzrealSpells[i].dmg = function(unit)
                    local Tard_level = Tard_myHero:GetSpellData(1).level
                    return CalcMagicalDamage(Tard_myHero, unit, ({70, 115, 160, 205, 250})[Tard_level] + 0.8 * Tard_myHero.ap)
                end
            elseif i == 3 then
                Tard_EzrealSpells[i].dmg = function(unit)
                    ...
                    elseif TardPred == 2 then
                        ...
                        local Tard_RPred = GetPred(unit, 2000, 1.76 + TardLatency() * .001)
                        if Tard_RPred then
                            Tard_Collision = mCollision(unit, 3, Tard_myHero.pos, Tard_RPred) + hCollision(unit, 3, Tard_myHero.pos, Tard_RPred)
                        end
                    end
...
-----------------------------------------------------<MENU>---------------------------------------------------
local Tard_Icon = {
    ['Sheen'] = 'http://raw.githubusercontent.com/yaddle/GosExt/master/Icons/Menu_Icons/Sheen.png',
