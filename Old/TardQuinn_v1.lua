...
function TardQuinn:Tard_AntiGapCloser()
    for i = 1, Game.HeroCount() do
        local Tard_Hero = Game.Hero(i)
        if Need:Tard_IsValidTarget(Tard_Hero,300) and Need:Tard_GetDistanceSqr(Tard_Hero.pos) > Need:Tard_GetDistanceSqr(Tard_Hero.posTo)  then
          self.Tard_CastE(Hero)
          print("antigapcloser working")         
        end 
      end
end
