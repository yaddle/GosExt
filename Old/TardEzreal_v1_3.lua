...
local Tard_Orb,
    ...
    Tard_SDKFlee,
    Tard_SDKSelector,
    Tard_SDKHealthPrediction,
    Tard_SDKDamagePhysical,
    Tard_SDKDamageMagical,
    Tard_CurrentTarget,
    Tard_SpellstoPred,
    Tard_Mode,
    Tard_TardMenu,
...
local Tard_version = 1.3
local Tard_Icon = {
    ["Ezreal"] = "https://raw.githubusercontent.com/yaddle/GosExt/master/Icons/EzrealSquare.png",
    ["Botrk"] = "https://raw.githubusercontent.com/yaddle/GosExt/master/Icons/Blade_of_the_Ruined_King_item.png",
    ["Cutlass"] = "https://raw.githubusercontent.com/yaddle/GosExt/master/Icons/Bilgewater_Cutlass_item.png"
}
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
class "Need"
...
function Need:Tard_IsValidTarget(unit, range)
    ...
    return unit and unit.isEnemy and unit.valid and Need:Tard_GetDistanceSqr(unit.pos) <= (range * range) and
        unit.visible and
...
function Need:Tard_HasBuff(unit, buffname)
    for i = 0, unit.buffCount do
        ...
        if
            Tard_Buff and Tard_Buff.name ~= "" and Tard_Buff.count > 0 and Game.Timer() >= Tard_Buff.startTime and
                Game.Timer() < Tard_Buff.expireTime and
                Tard_Buff.name == buffname
         then
...
function Need:CalcPhysicalDamage(source, target, amount)
    ...
    return math.max(
        0,
        math.floor(Need:DamageReductionMod(source, target, Need:PassivePercentMod(source, target, value) * amount, 1))
    )
end
...
function Need:CalcMagicalDamage(source, target, amount)
    ...
    return math.max(
        0,
        math.floor(DamageReductionMod(source, target, PassivePercentMod(source, target, value) * amount, 2))
    )
end
            ...
            if target:GetBuff(i).count > 0 then
                ...
                if target.charName == "MasterYi" then
                    if buff.name == "Meditate" then
                        amount =
                            amount -
                            amount * ({0.5, 0.55, 0.6, 0.65, 0.7})[target:GetSpellData(_W).level] /
...
function Need:PassivePercentMod(source, target, amount, damageType)
    ...
    if source.type == Obj_AI_Hero then
        if target.type == Obj_AI_Hero then
            if
                (Need:GetItemSlot(source, 3036) > 0 or Need:GetItemSlot(source, 3034) > 0) and
             ...
             then
                amount =
                    amount *
                    (1 +
                        math.min(target.maxHealth - source.maxHealth, 500) / 50 *
                            (Need:GetItemSlot(source, 3036) > 0 and 0.015 or 0.01))
            end
        end
    end
    return amount
...
function TardEzreal:__init()
    ...
    if _G.EOWLoaded then
        Tard_Orb = 0
        print("New Eternal Orb is good but Tosh is still toxic ^^")
    ...
    for i = 0, 3 do
        if i == 0 then
            Tard_EzrealSpells[i].dmg =
                ...
                return Need:CalcPhysicalDamage(
                    Tard_myHero,
                    unit,
                    ({35, 55, 75, 95, 115})[Tard_level] + 1.1 * Tard_myHero.totalDamage + 0.4 * Tard_myHero.ap
                )
            end
        elseif i == 1 then
            Tard_EzrealSpells[i].dmg = function(unit)
                local Tard_level = Tard_myHero:GetSpellData(1).level
                return Need:CalcMagicalDamage(
                    Tard_myHero,
                    unit,
...
function TardEzreal:Tard_Menu()
    ...
    Tard_TardMenu.Combo:MenuElement(
        {id = "ComboQmana", name = "Min. Mana to Q", value = 0, min = 0, max = 100, tooltip = "It's %"}
    )
    Tard_TardMenu.Combo:MenuElement(
        {id = "ComboWmana", name = "Min. Mana to W", value = 75, min = 0, max = 100, tooltip = "It's %"}
    )
    Tard_TardMenu.Combo:MenuElement({type = MENU, id = "Item", name = "Item"})
    Tard_TardMenu.Combo.Item:MenuElement(
        {id = "Botrk", name = "Blade of the Ruined King", value = true, leftIcon = Tard_Icon.Botrk}
    )
    Tard_TardMenu.Combo.Item:MenuElement(
        {id = "Cutlass", name = "Bilgewater Cutlass", value = true, leftIcon = Tard_Icon.Cutlass}
    )
    Tard_TardMenu.Combo.Item:MenuElement(
        {id = "MyHP", name = "Max HP to use items", value = 60, min = 0, max = 100, tooltip = "It's %"}
    )
    Tard_TardMenu.Combo.Item:MenuElement(
        {id = "EnemyHP", name = "Max enemy HP to use items", value = 60, min = 0, max = 100, tooltip = "It's %"}
    )
    ...
    Tard_TardMenu.Harass:MenuElement(
        {id = "HarassQMana", name = "Min. Mana to Q", value = 40, min = 0, max = 100, tooltip = "It's %"}
    )
    Tard_TardMenu.Harass:MenuElement(
        {id = "HarassWMana", name = "Min. Mana to W", value = 75, min = 0, max = 100, tooltip = "It's %"}
    )
    ...
    Tard_TardMenu.Farm:MenuElement(
        {id = "FarmMana", name = "Min. Mana", value = 70, min = 0, max = 100, tooltip = "It's %"}
    )
    ...
    Tard_TardMenu.LastHit:MenuElement(
        {id = "LastHitMana", name = "Min Mana To Lasthit", value = 40, min = 0, max = 100, tooltip = "It's %"}
    )
    ...
    Tard_TardMenu.JungleClear:MenuElement(
        {
            id = "JungleMana",
            name = "Min Mana To JungleClear",
            value = 60,
            min = 0,
            max = 100,
            step = 1,
    ...
    Tard_TardMenu.KS:MenuElement(
        {id = "R_Ksrange", name = "R Max Range", value = 7000, min = 300, max = 20000, step = 100, tooltip = "It's %"}
    )

    --[[Misc]]
    Tard_TardMenu:MenuElement({type = MENU, id = "Misc", name = "Misc Settings"})
    Tard_TardMenu.Misc:MenuElement(
        {
            id = "Rkey",
            name = "Ulti Champ targeted on key",
            key = string.byte("T"),
            tooltip = "the target need to be targeted by spell focus first, mouse clic on it, a blue circle should be on the target"
        }
    )
    Tard_TardMenu.Misc:MenuElement(
        {id = "KeepRmana", name = "Keep mana for R", value = false, tooltip = "KillSteal never keep mana"}
    )
    Tard_TardMenu.Misc:MenuElement(
        {id = "SelectedTarget", name = "Focus Spell target", value = true, tooltip = "Focus Spell on selected target"}
    )

    --[[Pred]]
    if Tard_EternalPred then
        Tard_TardMenu:MenuElement({type = MENU, id = "Pred", name = "Prediction Settings"})
        Tard_TardMenu.Pred:MenuElement(
            {
                id = "PredHitChance",
                name = "HitChance (default 25)",
                value = 25,
                min = 0,
                max = 100,
                tooltip = "higher value better pred but slower(%)||don't change it if don't know what is it||"
            }
        )
    end
    ...
    Tard_TardMenu.Draw:MenuElement(
        {
            id = "DrawReady",
            name = "Draw Only Ready Spells [?]",
            value = true,
            tooltip = "Only draws spells when they're ready"
        }
    )
    ...
    Tard_TardMenu.Draw:MenuElement(
        {
            id = "DrawSpellTarget",
            name = "Draw Spell Target Focus [?]",
            value = true,
            tooltip = "Draws spell target focus"
        }
    )
...
function TardEzreal:Tard_Tick()
    if Tard_myHero.dead then
        return
    end
    Tard_Mode = Need:Tard_GetMode()
    if
        (Tard_TardMenu.Misc.KeepRmana:Value() and Game.CanUseSpell(3) == 0 and Tard_myHero.mana >= 140) or
            (Game.CanUseSpell(3) ~= 0 or not Tard_TardMenu.Misc.KeepRmana:Value())
     then
...
function TardEzreal:Tard_Combo()
    local Tard_target
    if
        Tard_SelectedTarget ~= nil and
            (Tard_SelectedTarget.dead or Need:Tard_GetDistanceSqr(Tard_SelectedTarget.pos) > 2500 * 2500)
     then
        Tard_SelectedTarget = nil
    end
    ...
    if Tard_target == nil or Tard_myHero.attackData.state == 2 then
        return
    end
    --CAST Q SPELL
    if
        Tard_TardMenu.Combo.ComboQ:Value() and
            Need:Tard_PercentMP(Tard_myHero) >= Tard_TardMenu.Combo.ComboQmana:Value() and
            Game.CanUseSpell(_Q) == 0 and
            Need:Tard_IsValidTarget(Tard_target, 1200)
     then
        --CAST W SPELL
        self:Tard_CastQ(Tard_target)
        Tard_CurrentTarget = Tard_target
    elseif
        Tard_TardMenu.Combo.ComboW:Value() and
            Need:Tard_PercentMP(Tard_myHero) >= Tard_TardMenu.Combo.ComboWmana:Value() and
            Game.CanUseSpell(_W) == 0 and
            Need:Tard_IsValidTarget(Tard_target, 1050)
     then
    ...
    if Tard_TardMenu.Combo.Item.Botrk:Value() then
        ...
        if
            botrk >= 1 and Tard_myHero:GetSpellData(botrk).currentCd == 0 and
                Need:Tard_GetDistanceSqr(Tard_target.pos) <= 550 * 550 + (25 * 25) and
                Need:Tard_PercentHP(Tard_myHero) <= Tard_TardMenu.Combo.Item.MyHP:Value() and
                Need:Tard_PercentHP(Tard_target) <= Tard_TardMenu.Combo.Item.EnemyHP:Value()
         then
    ...
    elseif Tard_TardMenu.Combo.Item.Cutlass:Value() then
        ...
        if
            cutlass >= 1 and Tard_myHero:GetSpellData(cutlass).currentCd == 0 and
                Need:Tard_GetDistanceSqr(Tard_target.pos) <= 550 * 550 + (25 * 25) and
                Need:Tard_PercentHP(Tard_myHero) <= Tard_TardMenu.Combo.Item.MyHP:Value() and
                Need:Tard_PercentHP(Tard_target) <= Tard_TardMenu.Combo.Item.EnemyHP:Value()
         then
...
function TardEzreal:Tard_Harass()
    ...
    --CAST Q SPELL
    if
        Tard_TardMenu.Harass.HarassQ:Value() and
            Need:Tard_PercentMP(Tard_myHero) >= Tard_TardMenu.Harass.HarassQMana:Value() and
            Game.CanUseSpell(_Q) == 0 and
            Need:Tard_IsValidTarget(Tard_target, 1200)
     then
    ...
    --CAST W SPELL
    if
        Tard_TardMenu.Harass.HarassW:Value() and
            Need:Tard_PercentMP(Tard_myHero) >= Tard_TardMenu.Harass.HarassWMana:Value() and
            Game.CanUseSpell(_W) == 0 and
            Need:Tard_IsValidTarget(Tard_target, 1050)
     then
        if Tard_myHero.activeSpell.valid ~= true then
...
function TardEzreal:Tard_Farm()
    if
        not Tard_TardMenu.Farm.FarmQ:Value() or Need:Tard_PercentMP(Tard_myHero) < Tard_TardMenu.Farm.FarmMana:Value() or
            Tard_myHero.attackData.state == 2 or
            Game.CanUseSpell(0) ~= 0
     then
        return
    end
...
function TardEzreal:Tard_JungleClear()
    if
        not Tard_TardMenu.JungleClear.JungleQ:Value() or
            Need:Tard_PercentMP(Tard_myHero) < Tard_TardMenu.JungleClear.JungleMana:Value() or
            Tard_myHero.attackData.state == 2 or
...
function TardEzreal:Tard_LastHit()
    ...
    if
        not Tard_TardMenu.LastHit.LastHitQ:Value() or
            Need:Tard_PercentMP(Tard_myHero) < Tard_TardMenu.LastHit.LastHitMana:Value() or
            Tard_AAstate == 2 or
            Game.CanUseSpell(_Q) ~= 0
     then
        return
    ...
    for i = 1, Game.MinionCount() do
        local Tard_Minion = Game.Minion(i)
        if
            Tard_AAtarget ~= Tard_Minion.handle and Need:Tard_IsValidTarget(Tard_Minion, Tard_EzrealSpells[0].range) and
                Tard_EzrealSpells[0].dmg(Tard_Minion) >= Tard_Minion.health
         then
            if
                (Tard_AAstate == 3 and Need:Tard_HP_PRED(Tard_Minion, Tard_myHero.attackData.endTime - Game.Timer()) < 1) or
                    (Need:Tard_GetDistanceSqr(Tard_Minion.pos) >
                        (Tard_AArange + Tard_Minion.boundingRadius) * (Tard_AArange + Tard_Minion.boundingRadius) and
                        Tard_AAstate ~= 2)
             then
                self:Tard_CastQ(Tard_Minion)
                break
...
function TardEzreal:Tard_KillSteal()
    ...
    for i = 1, Game.HeroCount() do
        ...
        if Need:Tard_IsValidTarget(Tard_Hero) then
            ...
            if
                Tard_TardMenu.KS.Q_KS:Value() and Game.CanUseSpell(0) == 0 and
                    Need:Tard_GetDistanceSqr(Tard_Hero.pos) <= Tard_EzrealSpells[0].range * Tard_EzrealSpells[0].range
             then
                Tard_Q_DMG = Tard_EzrealSpells[0].dmg(Tard_Hero)
            end
            if
                Tard_TardMenu.KS.W_KS:Value() and Game.CanUseSpell(1) == 0 and
                    Need:Tard_GetDistanceSqr(Tard_Hero.pos) <= Tard_EzrealSpells[1].range * Tard_EzrealSpells[1].range
             then
                Tard_W_DMG = Tard_EzrealSpells[1].dmg(Tard_Hero)
            end
            if
                Tard_TardMenu.KS.R_KS:Value() and Game.CanUseSpell(3) == 0 and
                    Need:Tard_GetDistanceSqr(Tard_Hero.pos) <=
                        Tard_TardMenu.KS.R_Ksrange:Value() * Tard_TardMenu.KS.R_Ksrange:Value()
             then
            ...
            if
                Tard_Q_DMG ~= nil and Tard_W_DMG ~= nil and
                    Tard_Q_DMG + Tard_W_DMG > Tard_Hero.health + Tard_Hero.shieldAD + Tard_Hero.shieldAP
             then
                print("KS W+Q")
                Tard_CurrentTarget = Tard_Hero
                self:Tard_CastQ(Tard_Hero)
                DelayAction(
                    ...
                    Tard_EzrealSpells[0].delay + Game.Latency() / 1000
                )
            end
            if Tard_R_DMG and Tard_R_DMG > Tard_Hero.health + Tard_Hero.shieldAP then
                ...
                if
                    Need:Tard_GetDistanceSqr(Tard_Hero.pos) > Tard_AArange * Tard_AArange and
                        Need:Tard_HP_PRED(Tard_Hero, 1, 5)
                 then
...
function TardEzreal:Tard_CastQ(unit)
    if Tard_EternalPred == true then
        ...
        if
            Tard_QPred and (Tard_QPred.hitChance >= Tard_TardMenu.Pred.PredHitChance:Value() / 100) and
                Tard_QPred:mCollision() == 0 and
                Tard_QPred:hCollision() == 0 and
                Need:Tard_GetDistanceSqr(Tard_QPred.castPos) <= 1440000
         then
...
function TardEzreal:Tard_CastW(unit)
    if Tard_EternalPred == true then
        ...
        if
            Tard_WPred and Tard_WPred.hitChance >= Tard_TardMenu.Pred.PredHitChance:Value() / 100 and
                Need:Tard_GetDistanceSqr(Tard_WPred.castPos) < 1102500
         then
            if Need:Tard_IsValidTarget(unit) then
    ...
    function()
        if _G["TardEzreal"] and myHero.charName == "Ezreal" then
            ...
            if not _G.Prediction_Loaded then
                print(
                    "Warning : Eternal Prediction is missing and required, you need to install it, Ezreal script closing..."
                )
                return
            else
                _G["TardEzreal"]()
