...
local Tard_SDKCombo = _G.SDK.ORBWALKER_MODE_COMBO
local Tard_SDKHarass = _G.SDK.ORBWALKER_MODE_HARASS
local Tard_SDKJungleClear = _G.SDK.ORBWALKER_MODE_JUNGLECLEAR
local Tard_SDKLaneClear = _G.SDK.ORBWALKER_MODE_LANECLEAR
local Tard_SDKLastHit = _G.SDK.ORBWALKER_MODE_LASTHIT
local Tard_SDKFlee = _G.SDK.ORBWALKER_MODE_FLEE

function Need:Tard_GetMode()
		if Tard_Orb == "EOW" then
			return EOW:Mode()
		elseif Tard_Orb == "SDK" then			
			if Tard_SDKMode[Tard_SDKCombo] then
				return "Combo"
			elseif Tard_SDKMode[Tard_SDKHarass] then
				return "Harass"	
			elseif Tard_SDKMode[Tard_SDKLaneClear] or Tard_SDKMode[Tard_SDKJungleClear] then
				return "Clear"
			elseif Tard_SDKMode[Tard_SDKLastHit] then
				return "LastHit"
			elseif Tard_SDKMode[Tard_SDKFlee] then
				return "Flee"
			end
...
function TardQuinn:Tard_KillSteal()
  for i=1, Game.HeroCount() do
        ...
        if Need:Tard_GetDistanceSqr(Tard_hero.pos) <= (Tard_QuinnSpellQ.range*Tard_QuinnSpellQ.range) and Need:Tard_IsValidTarget(Tard_hero) then
          local Tard_Q_DMG = 0
          local Tard_E_DMG = 0
          local Tard_R_DMG = 0
...
function TardQuinn:Tard_CastQ(unit)
  if unit then
    ...
    local Tard_QCollision = unit:GetCollision(Tard_QuinnSpellQ.width,Tard_QuinnSpellQ.speed,Tard_QuinnSpellQ.delay)
      if Game.CanUseSpell(_Q) == 0 and Tard_QCollision == 0 and Need:Tard_GetDistanceSqr(myHero.pos,Tard_QPred) < (Tard_QuinnSpellQ.range*Tard_QuinnSpellQ.range) then
