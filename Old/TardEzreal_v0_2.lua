...
function Need:Tard_CastSpell(spell, pos, delay)
  local Tard_Delay = delay
  local Tard_SpellHitPosition = pos
  if counter == 0 then
    Tard_Mouse = mousePos
    counter = 1
  end
  if counter == 1 and myHero.activeSpell.valid ~= true then
    if Tard_SpellHitPosition:To2D().onScreen then  
          if mousePos:To2D() ~= Tard_SpellHitPosition:To2D() then
            Tard_Mouse = mousePos  
            print("souris déplacé")
            Control.SetCursorPos(Tard_SpellHitPosition)
            Control.KeyDown(spell)
            Control.KeyUp(spell)
            DelayAction(function()
              Control.SetCursorPos(Tard_Mouse)
              counter = 0
              end, Game.Latency()/1000)
          elseif mousePos:To2D() == Tard_SpellHitPosition:To2D() then
            print("bouge pas")
            Control.KeyDown(spell)
            Control.KeyUp(spell)           
          end      
      --Control.SetCursorPos(Tard_Mouse)
  --[[
    elseif myHero.activeSpell.valid ~= true then
      print("carte")
...
require("Eternal Prediction")
  ...
  if _G.Prediction_Loaded then self.Tard_EternalPred = true; print("Tosh Pred loaded ;)");
    ...
    self.test = Collision:SetSpell(self.Tard_EzrealSpells[0].range, self.Tard_EzrealSpells[0].speed, self.Tard_EzrealSpells[0].delay, self.Tard_EzrealSpells[0].width, self.Tard_EzrealSpells[0].hitBox)
  end  
  for i = 0, 3 do
  ...
  self.Tard_EzrealDmg = {
    [0] = function(unit) local Tard_level=myHero:GetSpellData(0).level return Need:CalcPhysicalDamage(myHero, unit, ({35, 55, 75, 95, 115})[Tard_level] + 1.1 * myHero.totalDamage + 0.4 * myHero.ap) end,
  ...
  print("lasthit")	
	for i = 1, Game.MinionCount() do
		...
		if Tard_AAtarget ~= Tard_Minion.handle and Need:Tard_IsValidTarget(Tard_Minion, self.Tard_EzrealSpells[0].range) and self.Tard_EzrealSpells[0].dmg(Tard_Minion) >= Tard_Minion.health then
			if Tard_AAstate == 3 or Need:Tard_GetDistanceSqr(Tard_Minion.pos) > Tard_AArange*Tard_AArange or GOS:CanAttack() then
				self:Tard_CastQ(Tard_Minion)
				break
			end
		end		
...
function TardEzreal:Tard_KillSteal()
	...
	for i = 1, Game.HeroCount() do
      ...
      if self.Tard_TardMenu.KS.R_KS:Value() and Game.CanUseSpell(3) == 0 and Need:Tard_GetDistanceSqr(Tard_Hero.pos) <= self.Tard_TardMenu.KS.R_Ksrange:Value()*self.Tard_TardMenu.KS.R_Ksrange:Value() then 
			...
			if Tard_W_DMG ~= nil and Tard_W_DMG > Tard_Hero.health + Tard_Hero.shieldAD then
				    print("KS W")
            print(Tard_W_DMG)
            self:Tard_CastW(Tard_Hero)
            Tard_CurrentTarget = Tard_Hero
            break
			elseif Tard_Q_DMG ~= nil and Tard_Q_DMG > Tard_Hero.health + Tard_Hero.shieldAD then
            print("KS Q")
            print(Tard_Q_DMG)
				    self:Tard_CastQ(Tard_Hero)
            Tard_CurrentTarget = Tard_Hero
            break
			elseif Tard_Q_DMG ~= nil and Tard_W_DMG ~= nil and Tard_Q_DMG + Tard_W_DMG > Tard_Hero.health + Tard_Hero.shieldAD then
            print("KS W+Q")
            Tard_CurrentTarget = Tard_Hero
            self:Tard_CastQ(Tard_Hero)
            DelayAction(function()
  ...
  if self.Tard_EternalPred == true then 
    local Tard_QPred = self.Tard_SpellstoPred[0]:GetPrediction(unit, myHero.pos)
      if Tard_QPred and (Tard_QPred.hitChance >= self.Tard_TardMenu.Pred.PredHitChance:Value()/100) and Tard_QPred:mCollision() == 0 and Tard_QPred:hCollision() == 0 then
        Need:Tard_CastSpell(HK_Q,Tard_QPred.castPos, self.Tard_EzrealSpells[0].delay)            
      end 
  else
    local Tard_QPred = unit:GetPrediction(self.Tard_EzrealSpells[0].speed, self.Tard_EzrealSpells[0].delay + Game.Latency()/1000)
    local Tard_QCollision, Tard_QblockUnit = self.test:__GetCollision(myHero, unit, 5)
    if (Tard_QCollision == false or (unit.type == Obj_AI_Minion and #Tard_QblockUnit == 1)) and Need:Tard_GetDistanceSqr(Tard_QPred) <= (self.Tard_EzrealSpells[0].range*self.Tard_EzrealSpells[0].range) then
...
function TardEzreal:Tard_CastW(unit)
  if self.Tard_EternalPred == true then
    ...
    if Tard_WPred and (Tard_WPred.hitChance >= self.Tard_TardMenu.Pred.PredHitChance:Value()/100) then
      Need:Tard_CastSpell(HK_W,Tard_WPred.castPos, self.Tard_EzrealSpells[0].delay)  
    end
  else
...
function TardEzreal:Tard_CastR(unit)
 ...
 --         if unit.pos:To2D().onScreen then
            ...
            Need:Tard_CastSpell(HK_R,Tard_RPred.castPos, self.Tard_EzrealSpells[3].delay)
--[[          else 
            print(Tard_RPred.castPos)
            local Tard_TargetMM = Tard_RPred.castPos:To2D()
            Control.CastSpell(HK_R, Tard_TargetMM.x, Tard_TargetMM.y)
            print("cast sur map !!!")
          end ]]
      end 
    else
