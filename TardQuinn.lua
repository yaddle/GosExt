Tard_Icon = {["Quinn"] = "https://vignette3.wikia.nocookie.net/leagueoflegends/images/a/af/Behind_Enemy_Lines.png"}
class "Need"
...
require("Eternal Prediction")		
	...
	self.Tard_QuinnDmg = {
		[0] = function(unit) local Tard_level=myHero:GetSpellData(0).level return Need:CalcPhysicalDamage(myHero, unit, (({20, 45, 70, 95, 120})[Tard_level] + ({0.8, 0.9, 1.0, 1.1, 1.2})[Tard_level] * myHero.totalDamage) + 0.5 * myHero.ap) end,
		[2] = function(unit) local Tard_level=myHero:GetSpellData(2).level return Need:CalcPhysicalDamage(myHero, unit, ({40, 70, 100, 130, 160})[Tard_level] + 0.2 * myHero.bonusDamage) end,
	...
	if _G.Prediction_Loaded then self.Tard_EternalPred = true; print("Tosh Pred loaded ;)"); self.Tard_QPred = Prediction:SetSpell(self.Tard_QuinnSpells[0], TYPE_LINE, true) end
  ...
  --CAST W SPELL  
	...
	-- CAST Q SPELL	
			if self.Tard_TardMenu.Combo.ComboQ:Value() and Tard_AAstate ~= 2  and Game.CanUseSpell(_Q) == 0 then  
				if Need:Tard_GetDistanceSqr(Tard_target.pos) > (Tard_AArange+myHero.boundingRadius + Tard_target.boundingRadius) * (Tard_AArange+myHero.boundingRadius + Tard_target.boundingRadius) or (Need:Tard_HasBuff(Tard_target, "QuinnW") == 0 or not self.Tard_TardMenu.Misc.Passive:Value()) and (Need:Tard_HasBuff(myHero, "QuinnR") == 0 or not self.Tard_TardMenu.Misc.NotQ_underR:Value()) then							
	...
	-- CAST E SPELL
			elseif self.Tard_TardMenu.Combo.ComboE:Value() and (Tard_AAstate == 3 or not self.Tard_TardMenu.Misc.E_AAreset:Value()) and Game.CanUseSpell(_E) == 0 then	      
				      if Need:Tard_GetDistanceSqr(Tard_target.pos) > (Tard_AArange+myHero.boundingRadius + Tard_target.boundingRadius)*(Tard_AArange+myHero.boundingRadius + Tard_target.boundingRadius) or (Need:Tard_HasBuff(Tard_target, "QuinnW") == 0 or not self.Tard_TardMenu.Misc.Passive:Value()) then
...
function TardQuinn:Tard_Vision(range)
  if Game.CanUseSpell(1) == 0 then
    for i=1, Game.HeroCount() do
          local Tard_Hero = Game.Hero(i)
          if Tard_Hero and Tard_Hero.team ~= myHero.team and not Tard_Hero.visible and not Tard_Hero.dead and Need:Tard_GetDistanceSqr(Tard_Hero.pos) <= (range*range) and Game.CanUseSpell(_W) == 0 then
